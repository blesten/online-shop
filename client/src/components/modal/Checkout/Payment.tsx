...
const Payment: React.FC<IProps> = ({ openPaymentModal, setOpenPaymentModal, paymentModalRef, subtotal, shipping, total, phone, user, shippingInformation, courier, service, item }) => {
  ...
  return (
    <div className={`${openPaymentModal ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} fixed top-0 left-0 bottom-0 right-0 bg-[rgba(0,0,0,.6)] flex items-center justify-center transition-opacity z-20 p-5`}>
